...
In order to carry out the exercise, one must first code the function to be approximated at arbitrary points.  The
required code is presented at the end of this m-file (see below). Let's begin:
"""
...
# values to create an Interpolator object with keyword argument y:
x = basis.nodes
y = f(x)
fa = Interpolator(basis, y=y)
print('Interpolation coeff =\n ', fa.c)  # attribute c returns the coefficients

# ... or one may simply pass the function directly to Interpolator using keyword 'f', which by default will evaluate it at the basis nodes
F = Interpolator(basis, f=f)
print('Interpolation coeff =\n ', F.c)
